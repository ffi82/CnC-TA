...
// @downloadURL   https://github.com/ffi82/CnC-TA/raw/master/C&C-TA_xTr1m_Base_Overlay.user.js
// @updateURL     https://github.com/ffi82/CnC-TA/raw/master/C&C-TA_xTr1m_Base_Overlay.user.js
// ==/UserScript==
(function () {
            ...
            qx.Class.define("xTr1m_Base_Overlay", {
                ...
                construct: function () {
                    try {
                        ...
                        var onKeyDown = function (e) {var xt = xTr1m_Base_Overlay.getInstance(); if (!!e.ctrlKey && !xt.__windowOpened) {switch (ClientLib.Vis.VisMain.GetInstance().get_Mode()) {case ClientLib.Vis.Mode.City:xt.__openWindow();}}};
                        var onKeyUp = function (e) {var xt = xTr1m_Base_Overlay.getInstance(); if (!e.ctrlKey && xt.__windowOpened) {switch (ClientLib.Vis.VisMain.GetInstance().get_Mode()) {case ClientLib.Vis.Mode.City:xt.__closeWindow();}}};
                ...
                members: {
                    ...
                    __window: null,
                    __openWindow: function () {this.__windowOpened = true; this.__window.open();},
                    __closeWindow: function () {this.__windowOpened = false; this.__window.close();}
				}
			});
                ...
                members: {
                    ...
                    open: function () {
                        ...
                        for (var i in hudEntities) {
                            ...
                            overlay._add(label);
                            this.__background._add(overlay, {left: entity.X + 5 * zoomFactor, top: entity.Y + 10 * zoomFactor});
						}
                        app.getDesktop().add(this, {left: mainOverlay.getBounds().left, top: mainOverlay.getBounds().top});
					},
                    close: function () {var app = qx.core.Init.getApplication(); app.getDesktop().remove(this);},
                        ...
                        try {
                            ...
                            for (var i in resList) {
                                ...
                                for (var b in resEntry) {
                                    ...
                                    if (!(index in this.__buildings)) {this.__buildings[index] = building;}
                                    else {this.__buildings[index].Gain += building.Gain;}
								}
							}
                            for (var j in this.__buildings) {this.__buildings[j].Ratio = this.__buildings[j].Gain / this.__buildings[j].Cost;}
						}
                        catch (e) {console.log("xTr1m_Base_Overlay.Window.collectData: ", e);}
					},
                    getResList: function (city, arTechtypes, eModPackageSize, eModProduction) {
                        try {
                            ...
                            var resAll = [];
                            var objbuildings = [];
                            if (PerforceChangelist >= 376877) {for (var o in buildings) {objbuildings.push(buildings[o]);}} //new
                            else {for (i = 0; i < buildings.length; i++) {objbuildings.push(buildings[i]);}} //old
                            for (i = 0; i < objbuildings.length; i++) {
                                ...
                                var bSkip = true;
                                for (var iTypeKey in arTechtypes) {if (arTechtypes[iTypeKey] == iTechType) {if(buildingLevel < maxLevel) {bSkip = false; break;}}} // only show where needed
                                if (bSkip) {continue;}
                                if (city_buildingdetailview === null) {continue;}
                                ...
                                for (var ModifierType in city_buildingdetailview.OwnProdModifiers.d) {
                                    switch (parseInt(ModifierType, 10)) {
                                        ...
                                        case eModProduction: {resbuilding.Gain += city_buildingdetailview.OwnProdModifiers.d[ModifierType].NewLvlDelta; break;}
									}
								}
                                for (var costtype in TechLevelData) {
                                    if (typeof (TechLevelData[costtype]) == "function" || TechLevelData[costtype].Type == "0") {continue;}
                                    if (parseInt(TechLevelData[costtype].Count) <= 0) {continue;}
                        ...
                        catch (e) {console.log("xTr1m_Base_Overlay.Window.getResList: ", e);}
					},
				}
			});
		}
            ...
            try {
                if (typeof qx != 'undefined' && typeof qx.core != 'undefined' && typeof qx.core.Init != 'undefined' && qx.core.Init.getApplication() !== null) {
                    ...
                    if (app.initDone === true) {
                        ...
                        catch (e) {console.log("xTr1m's Base Overlay script init error:"+e);}
					} else
