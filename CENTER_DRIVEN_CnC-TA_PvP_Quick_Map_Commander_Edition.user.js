// ==UserScript==
// @name        CENTER DRIVEN Command & Conquer TA PvP Quick Map: Commander Edition
// @description Creates a detailed map of bases and pois of the alliance and enemies.
// @namespace   https://prodgame*.alliances.commandandconquer.com/*/index.aspx*
// @include     https://prodgame*.alliances.commandandconquer.com/*/index.aspx*
// @version     1.5.1 BETA
// @grant none
// @author 	UPDATED by XDAAST | Orinially by zdoom
// ==/UserScript==
		...
		qx.Class.define("cdccta_map", 
		{
			...
			construct: function()
			{
				...
				{				
					...
					var mapButton = new qx.ui.form.Button('PvP Quick Map').set({ enabled: false });
...
qx.Class.define("cdccta_map.container",
		...
		construct: function()
		{
			...
			var canvasContainer = new qx.ui.container.Composite(new qx.ui.layout.VBox(1)).set({width:500, height:500});
			...
			var widget = new qx.ui.core.Widget().set({ width: 900, height: 800 });
			...
			var div = new qx.html.Element('div', null, {id: 'canvasContainer'});
					...
					var editAlliance = new qx.ui.form.Button('Edit Alliances').set({toolTipText:"Change the Alliances shown on Map", width:100, alignY:"top"});
					...
					grid.set({ minWidth: 1500, backgroundColor: '#415349', minHeight: 524, margin: 3, paddingTop: 10 });
					...
					mask.style.background = 'url("https://www.openmerchantaccount.com/img/map_mask.png") center center no-repeat';
		...
		//				console.log(base);
						...
						{
							var title = new qx.ui.basic.Label(base[i][0]).set({font: 'font_size_13_bold', textColor: '#FFFFFF'});
							var value = new qx.ui.basic.Label(base[i][1]).set({font: 'font_size_11', textColor: '#FFFFFF', marginBottom: 5});
							info.add(title);
							info.add(value);
						}
					}
				...
				__createLayout: function()
				{
					...
					var endGame = cdccta_map.getInstance().__endGame, hubs = endGame.hubs, fortress = endGame.fortress;
		...
		//			console.log(localStorage.cdccta_map_settings);
				...
				{
					var defaults = cdccta_map.getInstance().__defaultAlliances, items = this.__items, changed = false;
					if(!defaults) return;
				...
				{
					var map = cdccta_map.getInstance(), current = (map.__selectedAlliances == null) ? map.__defaultAlliances : map.__selectedAlliances;
