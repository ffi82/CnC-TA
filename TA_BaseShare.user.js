...
// @updateURL   https://github.com/ffi82/CnC-TA/raw/refs/heads/master/TA_BaseShare.meta.js
// @downloadURL https://github.com/ffi82/CnC-TA/raw/refs/heads/master/TA_BaseShare.user.js
// @grant       none
// ==/UserScript==
'use strict';
(function () {
    var b = function () {
            ...
            qx.Class.define("BaseShare", {
                ...
                members: {
                    ...
                    UpdateData: function () {
                        ...
                        for (var j = this.processingData.length - 1; j >= 0; j--) {
                            if (this.processingData[j][1] == -1) {
                                this.processingData.splice(j, 1);
                            }
                        }
                        this.StopScan();
                    },
        ...
        function checkIfLoaded() {
            try {
                if (typeof qx === 'undefined' || typeof qx.core.Init.getApplication !== 'function' || !qx?.core?.Init?.getApplication()?.initDone || !qx?.core?.Init?.getApplication()?.getMenuBar() || typeof ClientLib === 'undefined' || !ClientLib?.Data?.MainData?.GetInstance()?.get_Player()?.get_Id()) {
                    return setTimeout(checkIfLoaded, 1000);
                }
                createTweak();
                BaseShare.getInstance().Initialize();
                console.log(`%cBaseShare loaded`, 'background: #c4e2a0; color: darkred; font-weight:bold; padding: 3px; border-radius: 5px;');
            } catch (e) {
                console.error(`%cBaseShare error:`, 'background: black; color: pink; font-weight:bold; padding: 3px; border-radius: 5px;', e);
            }
        }
        checkIfLoaded();
    };
    try {
        ...
        console.error(`%cBaseShare init error:`, 'background: black; color: pink; font-weight:bold; padding: 3px; border-radius: 5px;', e);
    }
})();
