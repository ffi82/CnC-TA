		...
		function create_ccta_pa_class()
		{
			qx.Class.define('ccta_pa',
			{
				...
				construct: function()
				{
					...
					{
						this.base(arguments);
						this.set({layout: new qx.ui.layout.Grow(), label: "POI Manager", padding: 10});
						var root = this;
						...
						newFont.setSize(32);
						abr.setFont(newFont);
						//var deco = new qx.ui.decoration.Decorator().set({backgroundImage: ""});
						...
						var cont = new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({allowGrowY: true});
						var gb = new qx.ui.groupbox.GroupBox("Bonus Information").set({layout: new qx.ui.layout.VBox()});
						var lgb = new webfrontend.gui.GroupBoxLarge().set({layout: new qx.ui.layout.Canvas()});
						var lgbc = new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({padding: [10,10,20,10]});
						...
						for (var i = 0; i < 50; i++)
						{
							...
							var bonus = getBonus(startRank, score, multiplier);
							var percent = getBonus(endRank - 1, score, multiplier);
					...
					//Simulator
						...
						//var mainTabview = overlay.getChildren()[11].getChildren()[0];
 						var mainTabview = overlay.getLayoutChildren()[11].getLayoutChildren()[0];
				...
				members:
				{
					...
					__setRankingRows: function(score)
					{
						...
						var getRankingData = function(i, type, nr)
						{
							...
							var multiplier = ClientLib.Data.MainData.GetInstance().get_Server().get_POIGlobalBonusFactor();
							var bonus = getBonus(pois[type].index, score, multiplier);
						...
						{
							var multiplier = ClientLib.Data.MainData.GetInstance().get_Server().get_POIGlobalBonusFactor();
							var b = getBonus(pois[p].index, s, multiplier);
					...
					__updateGraph: function()
					{
						try
						{
							...
							for (var i = 0; i < ranks.length; i++)
							{
								...
								//needed multiplier updated and added below
								var nextBonus = getBonus(type, nextScore, multiplier);
								var totalBonus = ClientLib.Base.PointOfInterestTypes.GetTotalBonusByType(type, rank, score, multiplier);
								//needed multiplier updated and added above
